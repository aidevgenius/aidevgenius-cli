#!/bin/bash
# aidevgenius-flutter - Doctor Command

cmd_doctor() {
    print_header "aidevgenius-flutter Doctor"
    ...
    if is_flutter_installed; then
        ...
        echo "       Run: aidevgenius-flutter install flutter"
        all_ok=false
    fi
    ...
    if is_flutter_project; then
        ...
        if has_workflow_file; then
            print_success "aidevgenius-flutter workflow found"
        else
            print_warning "No aidevgenius-flutter workflow"
            echo "       Run: aidevgenius-flutter init"
        fi
        
        if config_exists; then
            print_success "Config file found: .aidevgenius-flutter.yaml"
        else
