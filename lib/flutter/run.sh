#!/bin/bash
# aidevgenius-flutter - Run Command (Web Server)
...
cmd_run() {
    ...
    case "$run_target" in
        ...
        *)
            ...
            echo "Usage: aidevgenius-flutter run web"
            exit 1
            ;;
    esac
}
...
cmd_run_web() {
    ...
    if ! is_flutter_installed; then
        die "Flutter not installed. Run 'aidevgenius-flutter install flutter' first."
    fi
