#!/bin/bash
# aidevgenius flutter - Build Command
...
cmd_build() {
    ...
    if [[ "$output_type" != "apk" || "$target_platform" != "all" ]]; then
        ...
        trigger_output=$(gh workflow run aidevgenius-build.yml \
        ...
        if [[ $? -eq 0 ]]; then
            ...
            if [[ "$trigger_output" == *"Unexpected inputs"* ]]; then
                ...
                echo ""
                echo "Run: aidevgenius flutter init"
                echo "Then: git add . && git commit -m 'Update workflow' && git push"
                echo ""
                echo "Or use simple build: aidevgenius flutter build apk --${build_type}"
                exit 1
            fi
        fi
    fi
    ...
    if [[ "$trigger_success" == "false" ]]; then
        trigger_output=$(gh workflow run aidevgenius-build.yml -f build_type="${build_type}" 2>&1)
    ...
    if [[ "$trigger_success" == "false" ]]; then
        ...
        echo "  1. Make sure workflow file exists: .github/workflows/aidevgenius-build.yml"
        echo "  2. Push the workflow: git push"
        echo "  3. Check GitHub authentication: gh auth status"
        echo "  4. Re-run init: aidevgenius flutter init"
        exit 1
    fi
    ...
    local run_id
    run_id=$(gh run list --workflow=aidevgenius-build.yml --limit=1 --json databaseId -q '.[0].databaseId' 2>/dev/null)
...
show_build_help() {
    echo ""
    echo "Usage: aidevgenius flutter build <type> [OPTIONS]"
    echo ""
