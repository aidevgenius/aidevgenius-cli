#!/bin/bash
# aidevgenius flutter - Init Command

cmd_init() {
    print_header "Initialize aidevgenius flutter"
    ...
    if [[ "$remote_url" != *"github.com"* ]]; then
        ...
        echo "aidevgenius currently only supports GitHub for cloud builds"
    ...
    local workflow_file=".github/workflows/aidevgenius-build.yml"
    ...
    rm -f ".github/workflows/aidevgenius-flutter-build.yml" 2>/dev/null

    # Copy workflow from aidevgenius template directory
    local template_file="${AIDEVGENIUS_HOME}/workflows/aidevgenius-flutter-build.yml"
    ...
    if [[ -f "$template_file" ]]; then
        ...
        curl -fsSL "https://raw.githubusercontent.com/aidevgenius/aidevgenius-cli/dev/workflows/aidevgenius-flutter-build.yml" -o "$workflow_file" 2>/dev/null
    ...
    # Rename workflow to aidevgenius-build.yml (standard name)
    if [[ -f "$workflow_file" ]]; then
        print_success "Created .github/workflows/aidevgenius-build.yml"
    fi
    
    # Create/update config file
    print_step "Creating configuration file..."
    
    cat > ".aidevgenius.yaml" << CONFIG_EOF
# aidevgenius configuration v${VERSION}
project:
  name: ${app_name}
  type: flutter
...
cloud:
  provider: github
  workflow: aidevgenius-build.yml
  poll_interval: 8
CONFIG_EOF
    
    print_success "Created .aidevgenius.yaml"
    
    # Add to .gitignore if not present
    if [[ -f ".gitignore" ]]; then
        if ! grep -q "# aidevgenius" .gitignore 2>/dev/null; then
            echo "" >> .gitignore
            echo "# aidevgenius" >> .gitignore
            echo ".aidevgenius-cache/" >> .gitignore
        fi
    fi
    ...
    echo "  • .github/workflows/aidevgenius-build.yml"
    echo "  • .aidevgenius.yaml"
    echo ""
    echo "Next steps:"
    echo "  1. Commit and push:"
    echo "     git add . && git commit -m 'Add aidevgenius cloud build' && git push"
    echo ""
    echo "  2. Build commands (same as flutter build):"
    echo ""
    echo -e "${CYAN}# APK builds${NC}"
    echo "     aidevgenius flutter build apk --debug"
    echo "     aidevgenius flutter build apk --release"
    echo ""
    echo -e "${CYAN}# Split APK (smaller per-device)${NC}"
    echo "     aidevgenius flutter build apk --release --split-per-abi"
    echo ""
    echo -e "${CYAN}# Target specific architecture${NC}"
    echo "     aidevgenius flutter build apk --release --target-platform android-arm64"
    echo ""
    echo -e "${CYAN}# App Bundle (for Play Store)${NC}"
    echo "     aidevgenius flutter build appbundle --release"
    echo ""
    echo -e "${CYAN}# Build and install on device${NC}"
    echo "     aidevgenius flutter build apk --release --install"
    echo ""
